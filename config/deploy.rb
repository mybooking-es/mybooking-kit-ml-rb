# config valid only for current version of Capistrano
lock '3.3.5'

set :application, 'kit'
set :repo_url, 'git@bitbucket.org:yuraksisa/repo.git'
set :deploy_to, '/home/developer/kit'
set :scm, :git

set :linked_files, fetch(:linked_files, []).push('config/unicorn.rb',
                                                 '.env')

set :linked_dirs, fetch(:linked_dirs, []).push('logs',
                                               'tmp/pids',
                                               'tmp/sockets',
                                               'public/.well-known')

#
# RVM configuration
#
set :rvm_type, :system
set :rvm_ruby_version, '2.4.1@mybooking-kit'
set :rvm_roles, [:app, :web]


#
# UNICORN configuration
#
set :unicorn_pid, "#{current_path}/tmp/pids/unicorn.pid"
set :unicorn_config_path, "#{current_path}/config/unicorn.rb"

# Default branch is :master
# ask :branch, proc { `git rev-parse --abbrev-ref HEAD`.chomp }.call

# Default deploy_to directory is /var/www/my_app_name
# set :deploy_to, '/var/www/my_app_name'

# Default value for :scm is :git
# set :scm, :git

# Default value for :format is :pretty
# set :format, :pretty

# Default value for :log_level is :debug
# set :log_level, :debug

# Default value for :pty is false
# set :pty, true

# Default value for :linked_files is []
# set :linked_files, fetch(:linked_files, []).push('config/database.yml')

# Default value for linked_dirs is []
# set :linked_dirs, fetch(:linked_dirs, []).push('bin', 'log', 'tmp/pids', 'tmp/cache', 'tmp/sockets', 'vendor/bundle', 'public/system')

# Default value for default_env is {}
# set :default_env, { path: "/opt/ruby/bin:$PATH" }

# Default value for keep_releases is 5
# set :keep_releases, 5

namespace :deploy do
  task :stop_unicorn do
  	invoke 'unicorn:stop'
  end
  task :start_unicorn do
    invoke 'unicorn:start'
  end
end

after "deploy:starting", "deploy:stop_unicorn"
after 'deploy:publishing', 'deploy:start_unicorn'
